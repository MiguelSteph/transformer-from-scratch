...
from fsdp_model_utils import shard_module_params
...
class MultiHeadAttentionModule(nn.Module):
    ...
    def setup(self):
        ...
        self.k_proj = sharded_dense(self.num_heads * self.d_k_proj)
        self.v_proj = sharded_dense(self.num_heads * self.d_v_proj)
        self.q_proj = sharded_dense(self.num_heads * self.d_k_proj)
        self.proj_back = sharded_dense(self.d_q)
...
class EncoderBlockModule(nn.Module):
    ...
    def __call__(self, inputs, mask=None, training=False):
        residual_x = inputs
        x = MultiHeadAttentionModule(self.num_heads, 
                                     self.emb_dim,
                                     self.d_proj, 
                                     self.d_proj, 
                                     self.data_axis_name, 
                                     self.min_weight_size
                                     )(inputs, inputs, inputs, mask)
        x = AddAndNormModule(self.dropout)(x, residual_x, training)

        residual_x = x
        x = FeedForwardModule(self.ff_d_inner, 
                              self.emb_dim, 
                              self.dropout, 
                              self.data_axis_name, 
                              self.min_weight_size
...
class DecoderBlockModule(nn.Module):
    ...
    def __call__(self, inputs, enc_output, dec_mask=None, enc_dec_mask=None, training=False):
        ...
        x = MultiHeadAttentionModule(self.num_heads, 
                                     ...
                                     self.data_axis_name, 
                                     self.min_weight_size
                                     )(enc_output, enc_output, x, enc_dec_mask)
        x = AddAndNormModule(self.dropout)(x, residual_x, training)
...
def create_transformer_module(config: ml_collections.ConfigDict, enc_pad_id: int, dec_pad_id: int) -> TransformerModule:
    return TransformerModule(
          ...
          data_axis_name=config.fsdp.data_axis,
